function [modout,errorval] = ColourCrowdPopModelFun(EP,MP,CalcErrVal)
%ColourCrowdPopModelFun
%
%function [modout] = ColourCrowdPopModelFun(ExParam,ModParam,CalcErrVal)
%model to predict the colour crowding data with responses at the population level
%
%v5.0 - same as MotionCrowdPopModelFun (see there for details)
%
%J Greenwood August 2018

modout.version = 5.0; %denote version of model here to be output with files for future reference
...
WFposDist = DrawGaussian(WFdim,0,MP.WFPosSD,MP.WFPosPeak,0); %values from 0 - WFPeak 
WFnegDist = DrawGaussian(WFdim,0,MP.WFNegSD,MP.WFNegPeak,0); %values from 0 - WFPeak 
...
for ff=1:EP.NumFlankVals
    ...
    for tar=1:EP.NumTarVals
        ...
        temppos = (repmat(DrawGaussianWrap(DirAxis,EP.FlankDirDiffs(ff),MP.ExciteSD,1,0),[MP.NumTrialsPerStim 1 1]) + (randn(MP.NumTrialsPerStim,numel(DirAxis)).*MP.LateNoise)).*((WFposVal(ff)).*ones(MP.NumTrialsPerStim,numel(DirAxis))); %FlankPeakResp+FlankNegMax,0); %(x,uEst,varEst,scaleEst,offsetEst)
        ...
        %work out proportion counterclockwise based on trials
        CrowdProbCCW(ff,tar) = sum((Cdirmax(tar,:,ff))>0)./numel(Cdirmax(tar,:,ff));%sum(temp(DirAxis>0))./sum(temp);%(sum(UncrowdModelResp,2)./NumTrialsPerStim)'; %makes final data in terms of proportion upwards
    end
end
...
if CalcErrVal
    ...
    data(1,:) = ([1.0342 1.1364    3.8719    5.0929   10.2376    6.8444    7.3398    2.4732   -8.1357   -7.6515  -11.3655   -3.4262   -2.1587    1.1364]-EP.BaseDir)./maxBias; %mean of n=6 bias values
    data(2,:)   = ([2.4770 1.0699    1.2537    1.5147    2.2605    1.8900    2.0145    2.0157    2.3159    2.4580    3.2540    1.6224    1.8921    1.0699])./maxTE;
    
    %using TE with UC threshold/bias
    datasim(1,:) = ([modout.UCmidpoint modout.Cmidpoint]-EP.BaseDir)./maxBias; %divide by max difference in actual data to put data in the same range ±1
    datasim(2,:) = ([modout.UCthreshold modout.ThreshElev])./maxTE; %divide by max threshold from real data to get in 0-1 range
