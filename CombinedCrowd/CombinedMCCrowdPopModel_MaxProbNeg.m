%CombinedMCCrowdPopModel_Neg
%model to predict Michael's motion-colour conjoint crowding data
%here with negative components in population for motion
%v1.1 based on CombinedPopModel 5.2 for both colour and motion
...
%Best thus far (fine fit, October 2018 v1):
% parameters:
...
%free parameters
ModParam.DirNoise  = 0.0120;
ModParam.ColNoise  = 0.0062;
ModParam.LateNoise = 0.0948;

NumFreeParam = 3;

%% run the model

[modout,errval] = CombinedMCCrowdPopModelFun_MaxProbNeg(ExParam,ModParam,DataPoolPropMean);
toc
