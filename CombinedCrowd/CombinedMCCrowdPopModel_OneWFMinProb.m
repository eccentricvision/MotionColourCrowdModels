%CombinedMCCrowdPopModel_OneWFMinProb
%model to predict Michael's motion-colour conjoint crowding data
%here with a single WF for both dimensions (still take the max if they differ)
%v5.2 for both colour and motion
%population processing version based on MotionCrowdPopModel.m and ColourCrowdPopModel.m
%
% NB this version essentially identical to Independent model but only MIN of WF is used
%
...
ModParam.ColWFNegPeak = ModParam.DirWFNegPeak;
ModParam.ColWFNegSD   = ModParam.DirWFNegSD;
ModParam.ColWFNegDelT = ModParam.DirWFNegDelT;

%free parameters
ModParam.DirNoise     = 0.0321;
ModParam.ColNoise     = 0.0061;
ModParam.LateNoise    = 0.1267;
ModParam.DirWFPosPeak = 0.5223;
ModParam.DirWFPosSD   = 103.4573;
ModParam.ColWFPosSD   = ModParam.DirWFPosSD;
ModParam.ColWFPosPeak = ModParam.DirWFPosPeak;

NumFreeParam = 5;

%% run the model

[modout,errval] = CombinedMCCrowdPopModelFun_MinProbNoNeg(ExParam,ModParam,DataPoolPropMean);
toc
...
for expt=1:4
    for cond=1:4
        ...
        if expt>1
            h(2) = plot(ExParam.DirAxis,modout.MeanFlankRespDir(expt,:,cond),'-','Color',colvals(2,:)); %flanker response
            h(3) = plot(ExParam.DirAxis,modout.MeanComboRespDir(expt,:,cond),'-','Color',colvals(3,:)); %combo response
...
for expt=1:4
    for cond=1:4
        ...
        plot([0 0],ylim,'k--'); %draw decision boundary
        if expt>1
            h(2) = plot(ExParam.DirAxis,modout.MeanFlankRespCol(expt,:,cond),'-','Color',colvals(2,:)); %flanker response
            h(3) = plot(ExParam.DirAxis,modout.MeanComboRespCol(expt,:,cond),'-','Color',colvals(3,:)); %combo response
