function [modout,errorval] = CombinedMCCrowdPopModelFun_MaxProbNoNeg(EP,MP,data)
...
DirFlankPeakTotal = 1;
DirFlankNegTotal  = 0; %turn off negative components
ColTarPeakTotal   = 1;
ColTarNegTotal    = 0;
ColFlankPeakTotal = 1;
ColFlankNegTotal  = 0; %turn of negative components
...
for expt=2:EP.NumExpts %should be 4 experiments (but first is uncrowded so skip - Just the 3 strong/weak crowded combos)
    for cond = 1:EP.NumConds(expt) %should be 1 4 4 4 for each experiment in terms of flanker condition
        ...
        %find the biggest and use the same for both
        DirWFposVal(expt,cond)  = max([DirWFposDist(DirWFind) ColWFposDist(ColWFind)]);
        DirWFnegVal(expt,cond)  = max([DirWFnegDist(DirWFind) ColWFnegDist(ColWFind)]);
        
        ColWFposVal(expt,cond)  = max([DirWFposDist(DirWFind) ColWFposDist(ColWFind)]);
        ColWFnegVal(expt,cond)  = max([DirWFnegDist(DirWFind) ColWFnegDist(ColWFind)]);
    end
end
