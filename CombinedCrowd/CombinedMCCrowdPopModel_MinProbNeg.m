%CombinedMCCrowdPopModel_MinProbNeg
%model to predict Michael's motion-colour conjoint crowding data
%v5.2 for both colour and motion;  does not use negative parameters
%population processing version based on MotionCrowdPopModel.m and ColourCrowdPopModel.m, both v5.1
%
% takes min prob instead of max for crowding on both dimensions
% includes inhibitory parameters
%
...
ModParam.DirNegSD     = 85.4421;

ModParam.DirWFPosPeak = 0.5069;
ModParam.DirWFNegPeak = 0.7007;
ModParam.DirWFPosSD   = 70.8827;
ModParam.DirWFNegSD   = ModParam.DirWFPosSD;
ModParam.DirWFNegDelT = 233.5278;
...
%free parameters
ModParam.DirNoise  = 0.0309;
ModParam.ColNoise  = 0.0114;
ModParam.LateNoise = 0.0634;

NumFreeParam = 3;

%% run the model

[modout,errval] = CombinedMCCrowdPopModelFun_MinProbNeg(ExParam,ModParam,DataPoolPropMean);
toc
